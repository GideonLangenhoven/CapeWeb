...
var YK = "sk_test_b1f890d6xW5kG4R72ab4732b2e87";
var CO = {"Access-Control-Allow-Origin":"*","Access-Control-Allow-Headers":"authorization, x-client-info, apikey, content-type","Access-Control-Allow-Methods":"POST, OPTIONS","Content-Type":"application/json"};
...
async function gemChat(hist,msg){
  try{
    ...
    var r=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key="+GK,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({system_instruction:{parts:[{text:"You are a friendly chat assistant for Cape Kayak Adventures in Cape Town. Keep responses SHORT (1-2 sentences). Sound like a real person. 1 emoji max.\nBusiness: Cape Kayak Adventures, Three Anchor Bay, Sea Point. Since 1994. 4.9 stars.\nTours: Sea Kayak R600/pp 90min, Sunset Paddle R600/pp 120min (sunset+wine).\nBring: Sunscreen, hat, sunnies, towel, change of clothes, water.\nMeet: Three Anchor Bay, Beach Rd, Sea Point. 15 min early. Free parking.\nSafety: Experienced guides, life jackets, stable kayaks. No exp needed. Ages 8+.\nCancel: 24h+ full refund. Under 24h reschedule/voucher. Weather=full refund.\nGroups: 6+ get 5% off. Vouchers: Any tour, 12 months.\nIf they want to book tell them to say book in this chat.\nNEVER make up times or dates."}]},contents:c,generationConfig:{temperature:0.7,maxOutputTokens:150}})});
    var d=await r.json();
...
async function getSlots(tourId,now){
  ...
  var{data}=await db.from("slots").select("id,start_time,capacity_total,booked,held").eq("tour_id",tourId).eq("status","OPEN").gt("start_time",now.toISOString()).lt("start_time",in30.toISOString()).order("start_time",{ascending:true}).limit(60);
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    var{data:allT}=await db.from("tours").select("*").order("base_price_per_person");
    var tours=(allT||[]).filter(function(t){return!t.name.includes("Private");});
    ...
    if(step==="IDLE"&&!isBtnClick){
      ...
      if(wGift){ns={step:"GIFT_PICK_TOUR"};reply="Awesome, gift vouchers make great presents! 🎁 Which tour should the voucher be for?";buttons=tours.map(function(t9){return{label:t9.name+" \u2014 R"+t9.base_price_per_person,value:t9.id};});return new Response(JSON.stringify({reply:reply,state:ns,buttons:buttons}),{status:200,headers:CO});}
      if(wLook){
        ...
        return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});
      }
      if(wBook||wAvail){
      ...
      return new Response(JSON.stringify({reply:reply,state:ns,buttons:buttons,calendar:calendar}),{status:200,headers:CO});
    }
    // ===== PICK_TOUR =====
    if(step==="PICK_TOUR"){
      var picked=null;
    ...
    if(step==="PICK_DATE"){
      if(isBtnClick&&btnVal.match(/^\d{4}-\d{2}-\d{2}$/)){
        ns={...ns,step:"PICK_TIME",selectedDate:btnVal};
        var slots3=await getSlots(ns.tid,now);
        var daySlots=slots3.filter(function(s3){return dateKey(s3.start_time)===btnVal;});
        ...
        }else{reply="No slots that day. Pick another date.";ns.step="PICK_DATE";}
      }else{
        ...
        if(slots4.length>0){
          ...
          reply="Pick a date from the calendar:";
        }else{reply="No slots available. Try another tour?";}
      }
      return new Response(JSON.stringify({reply:reply,state:ns,buttons:buttons,calendar:calendar}),{status:200,headers:CO});
    }
    // ===== PICK_TIME =====
    if(step==="PICK_TIME"){
      ...
      return new Response(JSON.stringify({reply:reply,state:ns,buttons:buttons}),{status:200,headers:CO});
    }
    // ===== ASK_QTY =====
    if(step==="ASK_QTY"){
      ...
      if(n>0&&n<=30){
        ...
        else{var tot=n*ns.tprice;var disc=0;if(n>=6){disc=Math.round(tot*0.05);tot=tot-disc;}ns={...ns,step:"ASK_DETAILS",qty:n,total:tot,baseTotal:n*ns.tprice,discount:disc};
          if(disc>0)reply=n+" people — nice group! You get 5% off (R"+disc+" saved). Total: R"+tot+". What's your full name?";
          else reply=pick([n+" people, awesome! What's your full name?",""+n+" — got it! And your name?"]);}
    ...
    if(step==="ASK_DETAILS"){
      ...
      if(!dName||!dEmail||!dPhone){var dMiss=[];if(!dName)dMiss.push("full name");if(!dEmail)dMiss.push("email");if(!dPhone)dMiss.push("phone number");reply="I still need your "+dMiss.join(", ")+". Please send all three, e.g.:\n\nJohn Smith, john@email.com, 082 123 4567";return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
    ...
    if(step==="ENTER_VOUCHER"){
      ...
      if(noVc){ns={...ns,step:"CONFIRM"};reply="No problem! Here's your booking summary:\n\n"+ns.tname+"\n"+ns.qty+" people \u2022 R"+ns.total+"\n\nReady to confirm?";buttons=[{label:"\u2705 Confirm & Pay R"+ns.total,value:"confirm"},{label:"\u274C Cancel",value:"cancel_booking"}];return new Response(JSON.stringify({reply:reply,state:ns,buttons:buttons}),{status:200,headers:CO});}
    ...
    if(step==="CONFIRM"){
      ...
      if(btnVal==="confirm"||lo.includes("yes")||lo.includes("confirm")||lo.includes("go ahead")||lo.includes("sure")||lo.includes("yep")){
        var ft=ns.total||0;
        var{data:bk}=await db.from("bookings").insert({business_id:ns.bid||tours[0]?.business_id,tour_id:ns.tid,slot_id:ns.slotId,customer_name:ns.name,phone:ns.phone||"",email:ns.email,qty:ns.qty,unit_price:ns.tprice,total_amount:ft,original_total:ns.baseTotal,status:"PENDING",source:"WEB_CHAT"}).select().single();
        if(!bk){reply="Something went wrong — try the Book Now page?";ns={step:"IDLE"};}
          ...
          reply="🎉 You're booked!\n\nRef: "+bk.id.substring(0,8).toUpperCase()+"\nConfirmation email on its way.\n\nMeet us at Three Anchor Bay — 15 min early. See you on the water! 🛶";ns={step:"IDLE"};
        }else{
      ...
      return new Response(JSON.stringify({reply:reply,state:ns,buttons:buttons,paymentUrl:pay}),{status:200,headers:CO});
    }
    // ===== LOOKUP =====
    if(step==="LOOKUP"){
    ...
    if(step==="RESCH_DATE"){
      // User picked a slot from calendar
      var rsId=btnVal||"";
      if(!rsId){reply="Please pick a date from the calendar above.";return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
      var{data:rsSlot}=await db.from("slots").select("id,start_time,booked").eq("id",rsId).single();
      if(!rsSlot){reply="Couldn\u2019t find that slot. Try again.";return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
      // Release old slot
      var{data:oldSl}=await db.from("slots").select("booked").eq("id",ns.slot_id).single();
      if(oldSl)await db.from("slots").update({booked:Math.max(0,oldSl.booked-ns.qty)}).eq("id",ns.slot_id);
      // Book new slot
      await db.from("slots").update({booked:rsSlot.booked+ns.qty}).eq("id",rsId);
      await db.from("bookings").update({slot_id:rsId}).eq("id",ns.booking_id);
    ...
    if(step==="CONFIRM_CANCEL"){
      ...
      ns={step:"IDLE"};return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});
    }

    // ===== MODIFY QTY =====
    if(step==="MODIFY_QTY"){
      var newQ=parseInt(msg);
      if(isNaN(newQ)||newQ<1||newQ>30){reply="Please enter a number between 1 and 30.";return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
      if(newQ===ns.current_qty){reply="That\u2019s the same! No changes needed \u{1F60A}";ns={step:"IDLE"};return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
      if(newQ>ns.max_avail){reply="Only "+ns.max_avail+" spots available. Try a smaller number.";return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
      ...
      if(qDiff>0){
        ...
        else{reply="Updated to "+newQ+" people! Contact us to arrange the extra R"+diffAmt+".";}}catch(e){reply="Updated but payment link failed.";}
      }else{
      ...
      ns={step:"IDLE"};return new Response(JSON.stringify({reply:reply,state:ns,paymentUrl:pay}),{status:200,headers:CO});
    }
    ...
    if(step==="CHANGE_TOUR"){
      ...
      if(!ctTour){reply="Please pick a tour.";return new Response(JSON.stringify({reply:reply,state:ns,buttons:buttons}),{status:200,headers:CO});}
      ...
      if(ctFit.length===0){reply="No available slots for "+ctTour.name+". Contact our team.";ns={step:"IDLE"};return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
      ...
      return new Response(JSON.stringify({reply:reply,state:ns,calendar:calendar}),{status:200,headers:CO});
    }
    ...
    if(step==="CHANGE_TOUR_SLOT"){
      ...
      if(!ctsSl){reply="Please pick a slot.";return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
      var{data:oldCtSl}=await db.from("slots").select("booked").eq("id",ns.slot_id).single();
      if(oldCtSl)await db.from("slots").update({booked:Math.max(0,oldCtSl.booked-ns.qty)}).eq("id",ns.slot_id);
      await db.from("slots").update({booked:ctsSl.booked+ns.qty}).eq("id",ctsId);
      var ctNewTot=ns.qty*Number(ns.new_price);
      await db.from("bookings").update({tour_id:ns.new_tour_id,slot_id:ctsId,unit_price:ns.new_price,total_amount:ctNewTot}).eq("id",ns.booking_id);
    ...
    // ===== UPDATE NAME =====
    if(step==="UPDATE_NAME"){
      if(msg.trim().length<2){reply="Please enter the new name:";return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});}
    ...
    if(step==="RESEND_CONFIRM"){
      if(btnVal==="resend_email"||lo.includes("yes")||lo.includes("resend")){
        try{await fetch(SU+"/functions/v1/send-email",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+SK},body:JSON.stringify({type:"BOOKING_CONFIRM",data:{email:ns.email,customer_name:ns.customer_name,ref:ns.booking_id.substring(0,8).toUpperCase()}})});
        reply="Sent! Check your inbox and spam folder \u2709\uFE0F";}catch(e){reply="Something went wrong. Please try again.";}
    ...
    return new Response(JSON.stringify({reply:reply,state:ns}),{status:200,headers:CO});
  }catch(err){console.error("ERR:",err);return new Response(JSON.stringify({reply:"Ah sorry, try that again?",state:{step:"IDLE"}}),{status:500,headers:CO});}
});
